...
class DevignUtil(object):
    ...
    # Compared with Reveal, it also requires adding NCS edges to the AST terminal nodes
    def generate_initial_node_info(self, ast: Dict) -> Data:
        ...
        # Add NCS edges
        # First find all AST terminal nodes in the graph and sort them in ascending order according to index order
        # Find the index of non-terminal nodes
        parent_idxs: Set[int] = set()
        terminal_idxs: List[int] = list()
        for edge in edges:
            parent_idxs.add(edge[1])
        # Find the index of the terminal node
