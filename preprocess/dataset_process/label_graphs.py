...
def process_sequence_datas(label_info: Dict[str, List[int]],
    ...
    for sequence in tqdm(sequences, desc="Labeling sequences"):
        ...
        line_nos: List[int] = sequence.get("line-Nos", None)
        skip_sequence = False
        for key in testcase_path.keys():
            if testcase_path[key] not in label_info.keys():
                if md5_value not in normal_md5_set:
                    normal_md5_set.add(md5_value)
                    normal_samples.append(sequence)
                skip_sequence = True
                break
            vul_line_nos: List[int] = label_info[testcase_path[key]]
            vul_idxs = []  # Khởi tạo một danh sách trống
            for i, line_no_list in enumerate(line_nos):
                for line_no in line_no_list:
                    if line_no in vul_line_nos:
                        vul_idxs.append(i)  # Thêm chỉ số vào danh sách nếu điều kiện được đáp ứng
            if not vul_idxs:
                if md5_value not in normal_md5_set:
                    normal_md5_set.add(md5_value)
                    normal_samples.append(sequence)
                ...
                if md5_value not in vul_md5_set:
                    vul_md5_set.add(md5_value)
                    sequence["tri_line_idxs"] = vul_idxs
                    vul_samples.append(sequence)
        if skip_sequence:
            continue

    both_md5_set = vul_md5_set & normal_md5_set
    final_normal_samples = [seq for seq in tqdm(normal_samples, desc="removing noise normal samples") if
                            get_sequence_md5(seq) not in both_md5_set]

    print("Labeling sequences done. Vulnerable sequence num: {}, normal sequence num: {}".format(len(vul_samples),
                                                                                                 len(final_normal_samples)))
    return vul_samples, final_normal_samples
